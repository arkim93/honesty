...
<head>
    ...
    <title>Laporan Detail Penjualan Barang</title>
</head>
...
<div id="header">
    <table width="100%" style="margin-top: 10px">
        <tr>
            <td class="judul_laporan">LAPORAN DETAIL PENJUALAN BARANG</td>
        </tr>
        <tr>
...
<table id="dataTableBuilder" class="table table-striped table-bordered table-hover display select tg"
    ...
    <tr>
        ...
        <th style="width: 9%" class="tg-3wr7"><strong>Discount</strong></th>
        <th style="width: 9%" class="tg-3wr7"><strong>Total</strong></th>
        <th style="width: 9%" class="tg-3wr7"><strong>Laba</strong></th>
        <th style="width: 9%" class="tg-3wr7"><strong>Rata</strong></th>
    ...
    @foreach($penjualan as $i => $value)
        <tr>
            ...
            <td style="width: 9%; text-align: right;" class="tg-rv4w">{{ number_format($value->discount, 0, ',', '.') }}</td>
            ...
            <td style="width: 9%; text-align: right;" class="tg-rv4w">{{ number_format($value->keuntunganrata(), 0, ',', '.') }}</td>
        ...
        <tr>
            ...
            <th style="text-align: center;" class="tg-3wr7-detail"><strong>H. Beli</strong></th>
            <th style="text-align: center;" class="tg-3wr7-detail"><strong>H. Jual</strong></th>
            ...
            <th style="text-align: center;" colspan="2" class="tg-3wr7-detail"><strong>Rata-Rata</strong></th>
        </tr>

        @foreach($value->penjualandetail as $detail)
            <tr>
                <td style="text-align: center;" class="tg-rv4w-detail"></td>
                <td style="text-align: center;" class="tg-rv4w-detail">{{ $detail->jumlah }}</td>
                <td colspan="2" class="tg-rv4w-detail">{{ $detail->barang->kode.' - '.$detail->barang->nama }}</td>
                <td style="text-align: right;" class="tg-rv4w-detail">{{ number_format($detail->harga_beli, 0, ',', '.') }}</td>
                <td style="text-align: right;" class="tg-rv4w-detail">{{ number_format($detail->harga_jual, 0, ',', '.') }}</td>
                <td style="text-align: right;" class="tg-rv4w-detail">{{ number_format($detail->harga_jual * $detail->jumlah, 0, ',', '.') }}</td>
                <td style="text-align: right;" class="tg-rv4w-detail">{{ number_format(($detail->harga_jual * $detail->jumlah) - ($detail->harga_beli * $detail->jumlah), 0, ',', '.') }}</td>
                <td style="text-align: right;" class="tg-rv4w-detail" colspan="2">{{ number_format(($detail->harga_jual_3 * $detail->jumlah) - ($detail->harga_beli * $detail->jumlah), 0, ',', '.') }}</td>
    ...
    <tr>
        ...
        <th style="width: 9%; text-align: right;" class="tg-3wr7">{{ number_format($keuntunganrata, 0, ',', '.') }}</th>
        <th class="tg-3wr7" colspan="2">&nbsp;</th>
    </tr>
    </tfoot>
</table>
